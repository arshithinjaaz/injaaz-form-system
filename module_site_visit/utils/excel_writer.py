...
# s3_client_download = None # No longer needed
...
# =================================================================
# --- REMOVED S3 DOWNLOAD FUNCTIONS ---
# Functions like download_s3_file_to_temp and get_sig_image_from_s3 
# are deleted as they are no longer required.
# =================================================================
...
def download_cloudinary_image_to_temp(url):
    ...
    This function is now used for both report photos AND signatures.
    """
    if not url:
        return None
...
def get_sig_image_from_url(url, name):
    ...
    This REPLACES get_sig_image_from_s3.
    """
    if not url:
...
def get_image_from_cloudinary(url, width, height, placeholder_text="No Photo"):
    """
    Loads report photo image from a Cloudinary URL. (Unchanged, relies on common downloader).
    """
    if not url:
...
def build_report_story(visit_info, processed_items):
    ...
    # --- 3. Report Items (Section 2 - Unchanged, uses CLOUDINARY URLs) ---
