#!/usr/bin/env python3
"""
Safe updater: replace occurrences of `cloudinary.version` with `get_cloudinary_version()`
...
def process_file(path: Path):
    ...
    if 'cloudinary.version' not in text:
        return False
    new_text = text
    # Add import if not present
    if IMPORT_LINE not in text:
        # Find the last import block end to insert import after imports
...
def main():
    ...
    if not modified:
        print('No occurrences of cloudinary.version found.')
    else:
